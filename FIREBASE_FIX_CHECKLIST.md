...
2. ❌ Firebase NOT initialized before `messaging()` is called
...
export async function initializeFirebase(): Promise<void> {
  await firebase.app(); // Auto-initializes with config files
}
```
...
async initialize(): Promise<void> {
  ...
  const authStatus = await messaging().requestPermission(); // Now safe
}
```
